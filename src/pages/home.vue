...
const navActiveIndex = ref("desc");
let step = 1
const productRef = ref<Element>();
const applyRef = ref<Element>();
const recousetRef = ref<Element>();
...
onMounted(() => {
  ...
  const sections = document.querySelectorAll('.navLink');
  ...
  const observerNav = new IntersectionObserver((entries) => {
    ...
    threshold: 0.9, // 当50%的section可见时触发
    // rootMargin: '0px 0px -50% 0px' // 调整触发区域
  });
  ...
  // const threshold = isMobile.value ? 1 : 0.5

  //    const fireRefObserver = new IntersectionObserver(
  //   (entries) => {
  //     entries.forEach((entry) => {
  //       if (entry.isIntersecting) {
  //           console.log('desc');
  //         navActiveIndex.value = 'desc';
  //       }
  //     });
  //   },
  //   {
  ...
  // );
  //  if (fireContentRef.value) {
  //   fireRefObserver.observe(fireContentRef.value);
  // }


  // const productRefObserver = new IntersectionObserver(
  //   (entries) => {
  //     entries.forEach((entry) => {
  //       if (entry.isIntersecting) {
  //           console.log('product');
  //         navActiveIndex.value = 'product';
  //       }
  //     });
  //   },
  //   {
  ...
  // );
  //  if (productRef.value) {
  //   productRefObserver.observe(productRef.value);
  // }


  //   const applyRefObserver = new IntersectionObserver(
  //   (entries) => {
  //     entries.forEach((entry) => {
  //       if (entry.isIntersecting) {
  //         console.log('apply');
  //         navActiveIndex.value = 'apply';
  //       }
  //     });
  //   },
  //   {
  ...
  // );
  //  if (applyRef.value) {
  //   applyRefObserver.observe(applyRef.value);
  // }

  //  const recousetRefObserver = new IntersectionObserver(
  //   (entries) => {
  //     entries.forEach((entry) => {
  //       if (entry.isIntersecting) {
  //           console.log('recouse');
  //         navActiveIndex.value = 'recouse';
  //       }
  //     });
  //   },
  //   {
  ...
  //   }
  // );
  //  if (recousetRef.value) {
  //   recousetRefObserver.observe(recousetRef.value);
  // }
...
const handleNavClick = (type: string) => {
  if (type === 'apply') {
    if (applyRef.value) {
      applyRef.value.scrollIntoView({ behavior: 'smooth' });
    }
  }
  if (type === 'desc') {
    if (fireContentRef.value) {
      fireContentRef.value.scrollIntoView({ behavior: 'smooth' });
    }
  }
  if (type === 'recouse') {
    if (recousetRef.value) {
      recousetRef.value.scrollIntoView({ behavior: 'smooth' });
    }
  }
  if (type === 'product') {
    if (productRef.value) {
  ...
  <div class="home-page" :class="{ mobile: isMobile }">
    <div class="header-nav">
      ...
      <div class="header-nav-list">
        <div class="header-nav-item" data-id="desc" :class="{ active: navActiveIndex === 'desc' }"
          @click="handleNavClick('desc')">{{ languageDic.nav.text1 }}</div>
        <div class="header-nav-item" data-id="product" :class="{ active: navActiveIndex === 'product' }"
          @click="handleNavClick('product')">{{ languageDic.nav.text2 }}</div>
        <div class="header-nav-item" data-id="apply" :class="{ active: navActiveIndex === 'apply' }"
          @click="handleNavClick('apply')">{{ languageDic.nav.text3 }}</div>
        <div class="header-nav-item" data-id="recouse" :class="{ active: navActiveIndex === 'recouse' }"
          @click="handleNavClick('recouse')">{{ languageDic.nav.text4 }}</div>
      </div>
    </div>
    <div class="fire-module" ref="fireModuleRef">
      <div class="fire-module-in" ref="fireContentRef">
        ...
        <div class="fire-module-content navLink" data-id="desc">
          <!-- <div class="header-nav">
            <div class="header-logo">
              <img src="@/assets/header-logo.png" alt="" />
            </div>
    ...
    <div class="my-product">
      <div class="my-product-in">
        <div class="my-product-title animationElement navLink"  data-id="product" :ref="(el) => handleContentItemRef(el)">
          {{ languageDic.product.title }}
        </div>
        <div class="my-product-content" ref="productRef">
          <div class="my-product-item animationElement" :ref="(el) => handleContentItemRef(el)"
            v-for="(item, index) in productListRef" :key="index">
            <div class="my-product-item-left">
              <div class="my-product-switch" v-if="item.posterList.length > 1">
                <div class="my-product-switch-item" :class="{ active: item.active === childIndex }"
                  @click="handleProductClick(item, childIndex)" :title="childItem"
                  v-for="(childItem, childIndex) in item.posterList" :key="childIndex">
                </div>

              </div>
              <div class="my-product-item-left-item" :style="{ height: childIndex === item.active ? '100%' : '0px' }"
                v-for="(childItem, childIndex) in item.posterList" :key="childIndex">
            ...
            <div class="my-product-item-right">
              ...
              <div class="my-product-item-right-body">
                <div class="my-product-item-right-body-item" v-for="(funcItem, funcIndex) in item.func"
                  :key="funcIndex">
                  <img class="my-product-item-right-body-item-icon" :src="getAssetsFile(funcItem.img)" alt="">
                  <p class="my-product-item-right-body-item-desc" v-for="(descItem, descIndex) in funcItem.decs"
                    :key="descIndex">{{ descItem }}</p>
                </div>
              </div>
    ...
    <div class="apply-module navLink" data-id="apply">
      <div class="apply-module-in" ref="applyRef">
    ...
    <div class="sdk-module">
      <div class="sdk-module-in">
        ...
        <div class="sdk-code" ref="codeRef" v-if="false">
          <div class="sdk-bg">
    ...
    <div class="frame">
      <div class="bottom navLink" data-id="recouse" ref="recousetRef">
        <div class="div-31">
          <div class="div-32">
            ...
            <div class="div-37">
              ...
              <div class="div-38">
                ...
                <div class="div-33">
                  ...
                  <div class="div-2">
                    <div class="text-wrapper-27">eric.zhangtx AT goertek.com</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="div-39">
          ...
          <div class="div-40">
            ...
            <div class="div-2">
              ...
              <div class="text-wrapper-30" style="display: flex; align-items: center;">
                <img class="police-icon" :src="getAssetsFile('police.png')" alt="">
                <a href="https://beian.mps.gov.cn/#/query/webSearch?code=37079402000603" target="_blank">{{ languageDic.footer.ICP }}</a>
              </div>
            </div>
          </div>
        </div>
